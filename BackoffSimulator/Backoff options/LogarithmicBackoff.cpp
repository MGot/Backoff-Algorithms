...
/***************CSTR & DSTR***********************/
LogarithmicBackoff::LogarithmicBackoff(long long size, unsigned long long amount) :
...
/***************GETTERS***********************/
unsigned long long LogarithmicBackoff::getAmountOfNodes()
{
    return amountOfNodes;
}

void LogarithmicBackoff::showNodes()
{
    for(auto i : nodes)
    {
...
long long LogarithmicBackoff::getRounds()
{
    return numberOfRounds;
}

unsigned int LogarithmicBackoff::countEnergyPoints()
{
    auto energyPoints = 0;
    for(auto i : nodes)
    {
...
unsigned long long LogarithmicBackoff::getRunningTime()
{
...
/***************SIMULATION FUNCT***********************/
void LogarithmicBackoff::startSimulation()
{
    ...
    while(!checkNodes())
    {
        ...
        tempSize = W->size();
        W->clear();
        W->resize(tempSize* (1 + 1/(ceil(log(tempSize)))), -1);
    }
    f = clock();
    //cout << endl << endl;
...
bool LogarithmicBackoff::checkNodes()
{
    for(auto i : nodes)
